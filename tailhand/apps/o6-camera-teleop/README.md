...
## reCamera 2002 腕部视角 MVP

首版把固定在手腕上的 reCamera 作为 RGB 网络相机使用。reCamera 只推送视频，Mac 继续负责物体检测、网页显示和唯一的 O6 控制器。本阶段不会根据画面自动移动 O6，也不会显示或估算厘米距离。

### 在 reCamera 开启 RTSP

1. 打开 `http://192.168.254.153/#/workspace`。
2. 在 Node-RED 工作区放置官方 `Camera` 和 `Stream` 节点，并连接 `Camera -> Stream`。
3. 将 Stream 配置为 RTSP、端口 `554`、session `live`；用户名和密码需与 `config.yaml > recamera > rtsp_url` 一致。默认示例使用 `admin/admin`，请按设备实际配置修改。
4. 点击 Deploy。该流程不应包含 CAN、O6 或其他机器人控制节点。
...
打开 `http://127.0.0.1:8765`，依次选择 `物品抓取 -> reCamera`。页面应显示腕部实时画面、物品框、一个靠近画面中心的高亮目标、RTSP FPS 和最新帧年龄。

reCamera 页面同时提供自动和手动两条验证路径。自动抓取按以下顺序操作：

1. 确认页面显示 RTSP 已连接、O6 后端正常、状态为 `未布防`，并让人的手离开黄色抓取区。
2. 点击 `布防识别`。布防本身不会移动 O6。
3. 把物品保持在画面中央和抓取区内；同一目标连续稳定 8 次检测后，状态从 `识别中` 进入 `正在抓取`。
4. O6 通过现有 EMA 和单次变化限幅逐步走向 `config.yaml > object_grasp > grasp_pose`，到达后保持抓取。
5. 释放必须点击 `安全张开`；目标离开画面不会自动松手。

`保守握合` 保留为不依赖识别计数的手动测试入口。dry-run 只在终端打印六维姿态，`紧急停止` 会锁存并拒绝后续动作。自动模式检测到目标丢失、目标跳变或人的手进入抓取区时，会在闭合前清零稳定计数。

本阶段只使用 RGB 目标框的位置和面积作为接近代理，不测量真实深度，也不把结果显示成厘米。reCamera 和 O6 的安装关系变化后，应重新确认抓取区与保守姿态。

确认画面与 dry-run 后才能启动真机：

```bash
./run_web.sh --real
```

按下 `布防识别` 或 `保守握合` 前，页面必须同时显示 `backend=mac-pcan-o6`、硬件已连接、`左手 / 0x28` 和 RTSP 已连接。若显示 `dry-run-fallback`，两个入口都会禁用，程序不会伪装成已经控制真机；显式 `--dry-run` 仍允许完整验证状态机。
