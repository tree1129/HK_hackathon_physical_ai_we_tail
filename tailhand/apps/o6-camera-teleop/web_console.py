...
def _update_rgb_grasp(
    ...
    observation = None if hand_blocked or target is None else _observation(target)
    return machine.update(observation)
...
class WebConsoleRuntime:
    def __init__(
        ...
        self._status = {
            ...
            "recamera_fps": 0.0,
            "recamera_frame_age_ms": None,
            "recamera_last_error": None,
            "depth_fps": 0.0,
    ...
    def _set_recamera_status(
        ...
        with self._status_lock:
            was_connected = bool(self._status["recamera_connected"])
            self._status.update(
                recamera_connected=connected,
                recamera_fps=fps,
                recamera_frame_age_ms=frame_age_ms,
                recamera_last_error=last_error,
            )
            if connected and not was_connected:
                self._status["last_event"] = (
                    "reCamera RTSP 已连接，可以布防自动抓取或手动验证"
                )
    ...
    def _apply_action(
        ...
        source_actions = {
            ...
            "source-recamera": VisionSource.RECAMERA,
        }
        if action in source_actions:
            source = source_actions[action]
            ...
            labels = {
                ...
                VisionSource.RECAMERA: "reCamera 腕部视角",
            }
            self._set_status(
                ...
                target_depth_mm=None,
                signed_distance_mm=None,
                last_event=f"已切换到{labels[source]}，当前未布防",
            )
            return safe_open.copy()
        ...
        if action == "follow-enable":
            ...
            if controller.emergency_stopped:
                ...
                mode_state.vision_source == VisionSource.RECAMERA
                and not status["recamera_connected"]
            ):
                self._set_status(last_event="reCamera RTSP 未连接，不能布防自动抓取")
            elif (
                mode_state.vision_source == VisionSource.RECAMERA
                and controller.backend == "dry-run-fallback"
            ):
                ...
                if (
                    mode_state.vision_source == VisionSource.MAC_CAMERA
                ...
                if mode_state.vision_source == VisionSource.RECAMERA:
                    self._set_status(
                        last_event=(
                            "已布防 reCamera；中央目标稳定 "
                            f"{machine.stable_frames} 帧后自动握合"
                        )
                    )
                else:
            ...
            elif not status["recamera_connected"]:
                self._set_status(last_event="reCamera RTSP 未连接，不能执行保守握合")
    ...
    def _run(self) -> None:
        recamera_config = self.config.get("recamera", {})
        rtsp_source = None
        if recamera_config.get("enabled", True):
            rtsp_source = RtspFrameSource(
                str(
                    recamera_config.get(
                        "rtsp_url",
                ...
                float(recamera_config.get("reconnect_interval_ms", 1000)) / 1000.0,
            )
        ...
        try:
            ...
            while not self._stop_event.is_set():
                ...
                if rtsp_source is not None:
                    rtsp_status = rtsp_source.status(now)
                    rtsp_fresh = (
                        rtsp_status.connected
                        and rtsp_status.frame_age_ms is not None
                        and rtsp_status.frame_age_ms
                        <= float(recamera_config.get("stale_timeout_ms", 1500))
                    )
                    self._set_recamera_status(
                        rtsp_fresh,
                        round(rtsp_status.fps, 1),
                        (
                            None
                            if rtsp_status.frame_age_ms is None
                            else round(rtsp_status.frame_age_ms, 1)
                        ),
                        rtsp_status.last_error,
                    )
                ...
                if sample is None:
                    ...
                    if mode_state.vision_source == VisionSource.IPHONE_LIDAR:
                        ...
                        if failures == 1:
                            self._set_status(last_event="reCamera RTSP 读取失败，正在重连")
                    else:
                ...
                if mode_state.mode == ConsoleMode.HAND_FOLLOW:
                    ...
                    if now - last_object_detection_time >= detector_period:
                        ...
                        if mode_state.vision_source == VisionSource.RECAMERA:
                            target = _closest_target_to_center(detections)
                        if (
                            target is None
